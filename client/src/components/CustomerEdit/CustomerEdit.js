...
const CustomerEdit = () => {
  ...
  const [customer] = useSelector((state) =>
    state?.customers.customers.filter((item) => item.id == id)
  );
  ...
  const Formik = useFormik({
    initialValues: {
      ...
      passwordrepeat: customer.passwordrepeat,
    },
    ...
    onSubmit: async (values) => {
      try {
        ...
        Formik.handleReset();
        alert.success(
          `${values.username}'s informations updated successfully!`,
          {
            timeout: 2000,
          }
        );
        ...
        <div className="right">
          ...
          <form onSubmit={Formik.handleSubmit}>
            ...
            <input
              ...
              value={
                Formik.values.lastlogin
                  ? moment(Formik.values.lastlogin).format(
                      "DD.MM.YYYY HH.mm.ss"
                    )
            ...
            {Formik.touched.createdAt && Formik.errors.createdAt && (
              <span>{Formik.errors.createdAt}</span>
            )}
